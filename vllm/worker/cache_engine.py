...
class FlattenedCacheEngine(CacheEngineBase):
    ...
    def _allocate_kv_cache_gpu(
        ...
        total_gpu_bytes = total_gpu_bytes / 1024 / 1024
        msg = f"GPU cache allocated {total_gpu_bytes} MB -> per layer {2 * flattened_kv_cache.numel()/self.num_attention_layers/1024/1024} MB"
        logger.info(msg)
        return kv_cache
    ...
    def _allocate_kv_cache_cpu(
        ...
        total_cpu_bytes = total_cpu_bytes / 1024 / 1024
        msg = f"CPU cache allocated {total_cpu_bytes} MB -> per layer {2 * flattened_kv_cache.numel()/self.num_attention_layers/1024/1024} MB"
        logger.info(msg)
        return kv_cache
