...
class FlattenedCacheEngine(CacheEngineBase):
    ...
    def _plan_cache_delta(self,
        ...
        if pause_and_resume:
            ...
            logger.debug(f"[RC] headroom={headroom} (min={HEADROOM_MIN})")

            free_now = snapshot.free_gpu_blocks + sum(len(v) for v in expected_freed.values())
            logger.debug(f"[RC] free_gpu_blocks={snapshot.free_gpu_blocks}, expected_freed_sum={sum(len(v) for v in expected_freed.values())}")
            alloc_need = sum(len(t[2]) for t in alloc_layers)
            if free_now < alloc_need + headroom:
                missing = alloc_need + headroom - free_now
            logger.debug("[RC] free_now=%d, alloc_need=%d, headroom= %d, missing=%d", free_now, alloc_need, headroom, missing)

            if missing > 0:
                logger.debug(f"[RC] Need {missing} additional blocks – scavenging paused seqs")
                ...
                if num_paused > 0:
                    ...
                    logger.debug(f"[RC] Will remove {blocks_per_seq} blocks from each paused sequence(num_paused={num_paused})")
                ...
                for sid in snapshot.paused_gpu_seqs:
                    ...
                    logger.debug("[RC] seq %d: will off‑load layers %s (free %d blocks)",
                    ...
                    logger.debug("[RC] After seq %d → remaining missing=%d", sid, missing)
                    if missing <= 0:
                        logger.debug("[RC] Target satisfied – stop scavenging")
                        logger.debug(f"mapping after scavenging: {self.mapping}")
                        break
                if missing > 0:
                    logger.debug("[RC] Still short of %d blocks after scavenging paused seqs", missing)
