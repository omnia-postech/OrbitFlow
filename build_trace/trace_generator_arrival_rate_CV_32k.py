...
def build_sched_save(
    ...
    for ap in arrival_patterns:
        ...
        trace = generate_trace(req_data, ap, arr_times.copy(), max_len=32_384, static=static)
        trace.num_gpu_blocks_override = fixed_blocks
        trace.add_estimate_sched(32_384, fixed_blocks, blk_size, req_data["batch_size"])
...
if __name__ == "__main__":
    ...
    for mix in mixes:                           # mixes_h9 + mixes_k12 + mixes_l20 ...
        ...
        save_request_json(
            path=f"traces/requests_types_32k_bs8/req_{tag}.json",      # traces 폴더에 저장
            request_types=probs,
            num_req=52,
            batch_size=8,
            skip_token_ids=True,
    ...
    for req_json in Path("traces/requests_types_32k_bs8").glob("req_*.json"):
        build_sched_save(
            request_json=str(req_json),
            blk_size=BLOCK_SIZE_TOK,
            static=False,
            skip_token_ids=True,
            arrival_rate_scales=[1.0, 2.0],  # req/s
            cvs=[1],  # Coefficient of Variation
        )
    # build_sched_save(
    #         request_json=str("/home/heelim/vllm/build_trace/traces/requests_types_32k/req_SS00_SM00_SL00_MS00_MM00_ML00_LS33_LM33_LL34.json"),
    #         blk_size=BLOCK_SIZE_TOK,
    #         static=False,
    #         skip_token_ids=True,
    #         arrival_rate_scales=[10.0],  # req/s
    #         cvs=[1]
    #     )
