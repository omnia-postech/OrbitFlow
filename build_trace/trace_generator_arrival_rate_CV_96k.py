...
def build_request_types_SML(
        short_bins=(512, 1024, 2048),
        mid_bins  =(2048, 4096, 8192),
        long_bins =(8192, 16384, 32768,
              65536) ,
        pct=0.3, max_total=96_384):
    span = lambda c: (max(16, int((c*(1-pct))//16*16)),
                      max(16, int((c*(1+pct))//16*16)))
...
if __name__ == "__main__":
    ...
    #     save_request_json(
    #         path=f"traces/sy_requests/req_{tag}.json",      # traces 폴더에 저장
    #         request_types=probs,
    #         num_req=52,
    #         batch_size=4,
    #         skip_token_ids=True,
    ...
    for req_json in Path("traces/sy_requests").glob("req_*.json"):
        build_sched_save(
