...
def build_sched_save(
    ...
    fixed_blocks = 2750  # Fixed GPU block budget (e.g., ~8 GB)
    ...
    for ap in arrival_patterns:
        ...
        trace = generate_trace(req_data, ap, arr_times.copy(), max_len=96_384, static=static)
        trace.num_gpu_blocks_override = fixed_blocks
        trace.add_estimate_sched(96_384, fixed_blocks, blk_size, req_data["batch_size"])
...
def build_request_types_SML(
        short_bins=(256, 512, 1024, 2048),
        mid_bins  =(4096, 6144, 8192, 12288),
        long_bins =(24576, 32768, 40960),
        pct=0.3, max_total=48000):
    span = lambda c: (max(16, int((c*(1-pct))//16*16)),
                      max(16, int((c*(1+pct))//16*16)))
...
if __name__ == "__main__":
    ...
    #     save_request_json(
    #         path=f"traces/requests_types_96k/req_{tag}.json",      # traces 폴더에 저장
    #         request_types=probs,
    #         num_req=52,
    #         batch_size=4,
    #         skip_token_ids=True,
    ...
    for req_json in Path("traces/requests_types_96k").glob("req_*.json"):
        build_sched_save(
