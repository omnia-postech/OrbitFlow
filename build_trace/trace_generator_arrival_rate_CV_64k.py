...
def build_sched_save(
    ...
    for ap in arrival_patterns:
        ...
        trace = generate_trace(req_data, ap, arr_times.copy(), max_len=64_384, static=static)
        trace.num_gpu_blocks_override = fixed_blocks
        trace.add_estimate_sched(64_384, fixed_blocks, blk_size, req_data["batch_size"])
...
def build_request_types_SML(
        short_bins=(256,  512,  1024, 2048),      # ≤ 2 K
        mid_bins  =(4096, 6144, 8192, 12288),     # 4 K–12 K
        long_bins =(16_384, 17_024, 17_408),         # 16 K–32 K
        pct=0.2, max_total=65536):
    span = lambda c: (max(16, int((c*(1-pct))//16*16)),
                      max(16, int((c*(1+pct))//16*16)))
...
if __name__ == "__main__":
    ...
    for mix in mixes:                           # mixes_h9 + mixes_k12 + mixes_l20 ...
        ...
        save_request_json(
            path=f"traces/requests_types_64k/req_{tag}.json",      # traces 폴더에 저장
            request_types=probs,
            num_req=52,
            batch_size=4,
            skip_token_ids=True,
    ...
    for req_json in Path("traces/requests_types_64k").glob("req_*.json"):
        build_sched_save(
