...
def build_sched_save(
    ...
    for ap in arrival_patterns:
        ...
        trace = generate_trace(req_data, ap, arr_times.copy(), max_len=131_072, static=static)
        trace.num_gpu_blocks_override = fixed_blocks
        trace.add_estimate_sched(131_072, fixed_blocks, blk_size, req_data["batch_size"])
...
def build_request_types_SML(
        short_bins=(256, 512, 1_024, 2_048),          # ≤ 2 K
        mid_bins  =(4_096, 6_144, 8_192, 12_288,
                    16_384),                          # 4 K–16 K        
        long_bins =(16_384, 20_480, 24_576, 26_624),  # 16 K–26 K
        pct=0.2, max_total=131_072):
    span = lambda c: (max(16, int((c*(1-pct))//16*16)),
                      max(16, int((c*(1+pct))//16*16)))
...
if __name__ == "__main__":
    ...
    for mix in mixes:                           # mixes_h9 + mixes_k12 + mixes_l20 ...
        ...
        save_request_json(
            path=f"traces/requests_types_128k/req_{tag}.json",      # traces 폴더에 저장
            request_types=probs,
            num_req=52,
            batch_size=4,
            skip_token_ids=True,
    ...
    for req_json in Path("traces/requests_types_128k").glob("req_*.json"):
        build_sched_save(
