...
MAX_REQ_TOK   = 24576    # 한 요청 input+output 상한
MAX_BATCH_TOK = 49152    # batch_size=4 가질 수 있는 총합 상한
BATCH_SIZE    = 4         # save_request_json 인자와 동일해야 함

def _valid_batch(reqs):
    """가장 큰 4개(request)의 토큰 총합이 96k 이하인지 검사."""
    top_sum = sum(sorted((r.input_length + r.output_length)
                         for r in reqs)[-BATCH_SIZE:])
    return top_sum <= MAX_BATCH_TOK
...
class RequestType:
    ...
    def _load_or_build_token_bank(self, name: str):
        path = f"/home/heelim/vllm/samples/{name}.json"
    ...
    def _build_sharegpt_bank(self, out_path) -> List[dict]:
        ...
        src = "/home/heelim/vllm/downloads/ShareGPT_V3_unfiltered_cleaned_split.json"
...
def build_sched_save(
    ...
    fixed_blocks = 1300  # Fixed GPU block budget (e.g., ~8 GB)
    ...
    for ap in arrival_patterns:
        ...
        trace = generate_trace(req_data, ap, arr_times.copy(), max_len=48192, static=static)
        trace.num_gpu_blocks_override = fixed_blocks
        trace.add_estimate_sched(48192, fixed_blocks, blk_size, req_data["batch_size"])
...
def build_request_types_SML(
        short_bins=(256, 512, 1_024, 2_048),  # ≤ 2K
        mid_bins=(4_096, 6_144),       # 4K–8K
        long_bins=(8_192, 9216, 9440),    # 8K–12K
        pct=0.30,
        max_total=12_288):
    span = lambda c: (max(16, int((c*(1-pct))//16*16)),
                      max(16, int((c*(1+pct))//16*16)))
...
if __name__ == "__main__":
    ...
    for mix in mixes:                           # mixes_h9 + mixes_k12 + mixes_l20 ...
        ...
        save_request_json(
            path=f"traces/requests_types_48k/req_{tag}.json",      # traces 폴더에 저장
            request_types=probs,
            num_req=52,
            batch_size=4,
            skip_token_ids=True,
    ...
    for req_json in Path("traces/requests_types_48k").glob("req_*.json"):
        build_sched_save(
