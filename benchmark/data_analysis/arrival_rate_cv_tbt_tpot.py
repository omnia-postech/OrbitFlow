...
# ───────────────────────────────────────────────
# 1. 설정 -------------------------------------------------------
method_list   = ["Flexgen", "SelectN", "NextLayer", "DistNSingleTD", "OursTD"]
method_labels = ["FlexGen", "SLO-aware Offloading", "DeepSpeed", "DistNSingle", "OrbitFlow"]
...
for i, sc in enumerate(slo_scales):
    ...
    for m, method in enumerate(method_list):
        ...
        #     print(f"{e}")
        #     y_tbt = [np.nan] * len(cv_rate)

        ax_TBT.plot(cv_labels, y_tbt, **style["line"],
        ...
        try:
            ...
            y_tpot = [np.nan] * len(cv_rate)

        ax_TPOT.plot(cv_labels, y_tpot, **style["line"],
    ...
    # row-label
    if i == 0:
        ax_TBT.set_ylabel("TBT SLO Attainment (%)", **style["label"])
        ax_TPOT.set_ylabel("TPOT SLO Attainment (%)", **style["label"])
    ...
    ax_TPOT.set_xlabel("CV Rate", **style["label"])
...
# 저장
plt.savefig("figures/arrival_rate_cv_tbt_tpot.jpg", bbox_inches="tight", dpi=300)
plt.savefig("figures/arrival_rate_cv_tbt_tpot.pdf", bbox_inches="tight")