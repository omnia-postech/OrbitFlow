...
# ───────────────────────────────────────────────
# 1. 설정 -------------------------------------------------------
method_list   = ["Flexgen", "SelectN", "NextLayer", "Ours"]
method_labels = ["FlexGen", "SLO-aware Offloading", "DeepSpeed", "OrbitFlow"]
...
for i, sc in enumerate(slo_scales):
    ...
    for m, method in enumerate(method_list):
        # TBT 데이터
        y_tbt = []
        summary_path = base_dir / f"slo{sc}" / method / "arrival_summerize.csv"
        try:
            ...
            for rate in arrival_rate:
                ...
                value = float(sel["tbt_attainment"].iloc[0]) if len(sel)==1 else np.nan
                y_tbt.append(value)
                # print(f"get {value}")
        except:
...
# 저장
plt.savefig("figures/arrival_rate_tbt_tpot_mean.jpg", bbox_inches="tight", dpi=300)
