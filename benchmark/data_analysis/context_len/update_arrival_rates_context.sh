...
# 공통 경로
base_path="/home/heelim/vllm/outputs/benchmark/paper_main_exp_context_length"  # change here
...
python /home/heelim/vllm/benchmark/data_analysis/data_parsing/change_slo_scale.py \
  ...
  --cv-list 1 \
  --methods Flexgen\
  --arrival-tpl "128k_lambda{rate}x_cv{cv}"   # change here
...
python /home/heelim/vllm/benchmark/data_analysis/context_len/make_arrival_rate_summerize_context_len.py "${all_roots[@]}"
if [ $? -ne 0 ]; then
...
python /home/heelim/vllm/benchmark/data_analysis/context_len/context_length.py "${base_path}"
if [ $? -ne 0 ]; then
