...
fig = plt.figure(figsize=(45, 5.5 * N_METRIC))
gs  = GridSpec(
    ...
    for c, trace in enumerate(trace_list):      # column pair
        ...
        for m, (method, m_label) in enumerate(zip(method_list, method_labels)):
            ...
            for idx, sc in enumerate(slo_scales):
                try:
                    df = load_metrics(Path(f"/home/heelim/vllm/outputs/benchmark/exp/{method}/{trace}_{metric}/{sc}/output.csv"))
        ...
        if c == N_TRACE - 1:
            ...
            # ax_L.yaxis.set_label_position("right")
            ax_R.set_ylabel("SLO attainment (%)", labelpad = style["label"]["labelpad"], fontsize=28)
            ax_R.yaxis.set_label_position("right")
...
# ───────────────────────────────────────────────
# 5. 중앙 세로선, 범례, 공통 텍스트 ---------------
fig.add_artist(Line2D([0.52, 0.52], [0.0, 0.92],
...
fig.text(0.31, -0.055, "TPOT", ha='center',
         fontsize=35, weight='bold')
fig.text(0.715, -0.055, "TBT", ha='center',
         fontsize=35, weight='bold')
